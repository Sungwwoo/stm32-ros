...
namespace mesh_msgs
{
  ...
  class MeshGeometry : public ros::Msg
  {
    public:
      ...
      typedef mesh_msgs::TriangleIndices _faces_type;
    ...
    virtual int deserialize(unsigned char *inbuffer)
    {
      ...
      if(faces_lengthT > faces_length)
        this->faces = (mesh_msgs::TriangleIndices*)realloc(this->faces, faces_lengthT * sizeof(mesh_msgs::TriangleIndices));
      faces_length = faces_lengthT;
      for( uint32_t i = 0; i < faces_length; i++){
      offset += this->st_faces.deserialize(inbuffer + offset);
        memcpy( &(this->faces[i]), &(this->st_faces), sizeof(mesh_msgs::TriangleIndices));
      }
     return offset;
    }
